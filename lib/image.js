...
async function imageFromScreenshot (screenshot, size, logger) {
  const image = await loadImage(Buffer.from(screenshot, 'base64')); //`data:image/png;base64,${screenshot}`;
  ...
  if (image.width !== size.width) {
    logger.info(`Screenshot and screen size did not match. Screen size is ` +
                ...
                `${image.width}x${image.height}. Scaling screenshot to match screen size`);
  }
