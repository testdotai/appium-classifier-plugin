...
async function imageFromScreenshot (screenshot, size, logger) {
  ...
  if (img.width === size.width) {
    await fs.writeFile('/Users/jlipps/Desktop/screen.png', Buffer.from(screenshot, 'base64'));
    return img;
  }
  ...
  await fs.writeFile('/Users/jlipps/Desktop/screen.png', newScreenshot);
...
async function elementImageFromScreenshot (el, screenshotImg) {
  ...
  await fs.writeFile(`/Users/jlipps/Desktop/elements/${el.ELEMENT} - (${x},${y}) ${width}x${height}.png`, canvas.toBuffer('image/png'));
