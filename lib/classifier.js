...
async function getMatchingElements (elsAndImages, label, confidence, allowWeakerMatches, logger) {
...
async function findByObjectDetector (driver, logger, label, allowWeakerMatches) {
  ...
  const screenshotPath = "/Users/jlipps/Desktop/test.png";
  ...
  const candidates = await detect(screenshotPath, driver.opts.testaiObjDetectionThreshold || 0.95, true);
...
async function find (driver, logger, label, /* multiple */) {
  ...
  if (driver.opts.testaiFindMode === OBJ_DETECTION_MODE) {
